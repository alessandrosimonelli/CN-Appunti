\section{Sistemi di Comunicazione}
    Un sistema di comunicazione numerico può essere approssimato a
    \begin{figure}[H]
        \centering 
        \begin{tikzpicture}[
            node distance=3cm,
    ...
    \subsection{Phase Amplitude Modulation - PAM}
        É una tipologia di sistema di comunicazione in {\color{blue}Banda Base}, il cui segnale trasmesso $s_{T(t)}$ ha densità spettrale
        ...
        \subsubsection{Modulatore}
            Il segnale PAM è una serie di impulsi che si susseguono in successione con un intervallo $T = \frac{1}{f_s}$ e modulatori in ampiezza dei simboli 
            ...
            \end{figure}
            dovendo analizzare il dominio frequenziale di $s_{T(t)}$ sarebbero tutte delle $sinc$ in {\color{blue}BB}  
        \subsubsection{Densità spettrale di potenza}\label{Densita spettrale di potenza - PAM}
            ...
            \subsubsection{Potenza}
                ...
                \begin{figure}[H]
                    ...
                    \end{tikzpicture}
                \end{figure}
                dalla quale possiamo ricavare la densità spettrale di potenza dei simboli
                \[
        ...
        \subsubsection{Rimozione dell'ISI}
            ...
            \end{figure}
        \paragraph{Dimensionamento di $g_{(t)}$}
            ...
            \end{itemize}
            in realtà non uso la $rect/sinc$ per realizzare il dimensionamento della $g_{(t)}$ 
            \begin{itemize}
    ...
    \subsection{Filtro adattato}
        ...
        \subsubsection{Progettazione dei filtri di ricezione e trasmissione}
            ...
            \]
            dato che la funzione coseno rialzato è reale e non negativa posso esprimere la $G_{T(f)}$ come
            \[
    ...
    \subsection{Circuito decisore}
        ...
        \subsubsection{Calcolo della SER per un sistema PAM M-ario}
            ...
            \begin{figure}[H]
                ...
                \end{tikzpicture}
            \end{figure}                    
            dove $z_{[k]} = \frac{x_{[k]}}{A} = a_k + \frac{n_{[k]}}{A}$ il rumore è quindi $\simeq\mathcal{N}(0,\frac{\sigma^2_n}{A^2})$, se calcoliamo
            ...
            \] 
            Se non scalassi per il fattore $A$ le zone di decisione dovrebbero adattarsi al valore di $A$.
            Il decisore suddivide l'asse $x$ in zone di decisione, come mostrato in figura 131, con le soglie poste esattamente
            a metà tra due simboli adiacenti nel caso di simboli equiprobabili
            \begin{figure}[H]
                \centering
                \begin{tikzpicture}
                    \begin{axis}[
            ...
            \end{figure}
            usando il teorema della probabilità totale possiamo esprimere la SER (Symbol Error Rate)
            \[
            ...
            \]
            restano quindi da calcolare le $M$ probabilità condizionate $P[r|a^{(m)}]$ per $m = 1,\dots,M$.
            L'analisi dell'alfabeto $\mathcal{A}$ e le zone di decisione si può evincere come i simboli 
            ...
            \begin{enumerate}
                \item {Calcolo di $P[e|a_k = 1]$
                    Supponiamo sia stato inviato il simbolo $a_k = 1$ il campione ricevuto è 
                    \[
                        z_{[k]} = 1 + \eta_{[k]}  
                    \]
                    \begin{figure}[H]
                        ...
                        \end{tikzpicture}
                    \end{figure}
                    notiamo come l'errore è presente quando il simbolo non è compreso all'interno dell'intervallo
                    \begin{align}
                        P[e|a_k =1] &= P_r[z_{[k]}< 0 \cup z_{[k]}> 2] = P_r[1+\eta_{[k]}< 0 \cup 1+\eta_{[k]}> 2] \nonumber \\
                                    &= P_r[\eta_{[k]}< -1 \cup \eta_{[k]}> 1]\nonumber 
                    \end{align}
                    potremmo calcolare l'integrale della distribuzione di probabilità ma utilizziamo la funzione $Q$,
                    gli intervalli di errore esterni sono uguali quindi possiamo calcolare $Q$ solo nel punto destro
                    \[
                ...
                \item {Calcolo di $P[e|a_k = M-1]$
                    Supponiamo di aver trasmesso il simbolo $a_k = M-1$, il campione è caratterizzato da
                    \[
                        z_{[k]} = M-1+\eta_{[k]}    
                    \]
                    \begin{figure}[H]
        ...
        \subsubsection{Efficienza spettrale ed efficienza energetica}
            ...
            \paragraph{Efficienza Spettrale: }\index{Efficenza Spettrale}\label{Efficenza Spettrale} l'efficienza spettrale del sistema PAM è definita da 
                \[
                ...
                dell'alfabeto. 
            \paragraph{Efficienza Energetica: }\index{Efficenza Energetica}\label{Efficenza Energetica}
        ...
        \subsubsection{Codifica GRAY nel sistema PAM}
            ...
            dal rapporto $\frac{E_s}{N_0}$ e da $M$, dipende anche dalla legge di mappatura. É però possibile 
            ...
            \] 
    \subsection{QAM}
        É un sistema di comunicazione in {\color{red}banda passante} in cui il segnale trasmesso $s_{T(t)}$ ha densità 
        ...
        \subsubsection{Trasmettitore - QAM}
            ...
            \end{figure}
        \subsubsection{Densità spettrale di potenza - QAM}
        ...
        \subsubsection{Ricevitore - QAM}
            ...
            indipendenti essendo congiuntamente Gaussiani. Dopo il campionatore abbiamo i campioni $x_{c[k]}$ e $x_{s[k]}$ espressi da 
            \begin{gather}
                x_{c[k]} = a_{[k]}+n_{c[k]}\nonumber \\
                x_{s[k]} = b_{[k]}+n_{s[k]}\nonumber
            \end{gather}
    ...
    \subsection{Modulazione Quadrature Amplitude Modulation - QAM}
        ...
        \end{figure}
        in cui a ciascun simbolo $c_i = a_i +jb_i$ è associata una coppia di bit. Se le mappe PAM sono quaternarie si ha un sistema 
        ...
        \begin{figure}[H]
            ...
            \caption{Ricevitore QAM}
        \end{figure}
        Come possiamo vedere la decisione non è presa sul simbolo complesso $c_k$, ma sui simboli $a_k$ e $b_k$ in modo 
        ...
        è presente l'interferenza dovuta al simbolo $a_k$. É quindi necessario effettuare una buona sincronizzazione,  
