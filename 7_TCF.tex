\section{Trasformata Continua Di Fourier}
    \subsection{Segnali Aperiodici}
        Nel caso di segnali come $x_{(t)}=rect\left(\frac{t}{T}\right)$ non posso usare la $TSF$ posso però scrivere:
        \[
    ...
    \subsection{Equazioni di Analisi e Sintesi}
        ...
        La $TCF$ gode della biunivocità
        \begin{align}
            x_{(t)} \overunderset{TCF}{ATCF}{\leftrightharpoons}  X_{(f)}\nonumber \hspace{0.3cm} X_{(f)} \in \mathbb{C}
        \end{align}
        Essendo $X_{(f)}$ un numero complesso può essere rappresentato in forma polare: 
        \[
        ...
        \end{figure}
        lo spettro di Ampiezza gode della \textbf{simmetria pari} rispetto alle ascisse quindi è \textbf{sempre positivo e continuo}, mentre lo spettro di fase della \textbf{simmetria dispari}, 
        questa proprietà è chiamata \textbf{Simmetria Hermitiana}
    ...
    \subsection{Proprietà}
        Come per la TSF vale che al variare del periodo della funzione $T$:
            ...
            \end{itemize}
        Inoltre come si può evincere dal successivo Teorema della Dualità \ref{Dualita}:
            \begin{itemize}
            ...
            \end{itemize}
        Da come vedremo più avanti quindi effettuare troncamenti in frequenza comporta, per alcuni segnali, perdere componenti ad alte
        frequenze, la scelta dell'intervallo di troncamento è cruciale per la ricostruzione del segnale: se tronchiamo un segnale perdendo
        ...
        \subsubsection{Parità}\label{Parita}
            \begin{align}
                Ip&: x_{(t)}\ reale\ e\ pari  \nonumber \\
                Th&: X_{(f)}\ reale\ e\ pari \nonumber  
            \end{align}

        \subsubsection{Disparità}\label{Disparita}
            \begin{align}
    ...
    \subsection{Teoremi relativi alla TCF}
        \subsubsection{Linearità}\label{Linearita}
        ...
        \subsubsection{Dualità}\label{Dualita}
        ...
        \subsubsection{Ritardo}\label{Ritardo}
            ...
            {\em Osservazione:}
                \begin{itemize}
                    \item Un ritardo nel tempo introduce una componente solo di fase che cresce linearmente con la frequenza
            ...
            Esempio:\\
                {
                    ...
                    \end{figure}
                    Il \LaTeX{} sbaglia e aggiunge le spike nelle $f$ positive, il grafico è dispari con andamento come per le $f$ negative.
                }
        ...
        \subsubsection{Derivazione}\label{Derivazione}
            ...
            \end{align}
            Posso Scrivere $y_{(t)}$ come $ACTF[y_{(t)}] = \int_{-\infty}^{\infty} Y_{(f)}e^{j2\pi ft} df $, se quindi $Y_{(f)} = j2\pi f X_{(f)}$ l'uguaglianza è valida:
            \[
        ...
        Esempio: $TCF$ di una piramide\\
        {
            ...
            \begin{itemize}
                ...
                \item {
                    ...
                    \end{gather} 
                    I segni negativi spariscono per il valore assoluto e per la parità della $rect$. Per verificare che i calcoli siano
                    corretti posso calcolare la $X_{(f)}$ in $0$ e vedo quanto è l'area del segnale: 
                    \[
                    ...
                    quindi lo spetto non occupa le alte frequenze, l'andamento è di $\frac{sinc^2}{x^2}$, è molto più contenuto. La rect avendo un gradino
                    varia molto rapidamente nel {\em tempo} e di conseguenza il suo spettro si estende a frequenze più alte del segnale piramidale.
                    \begin{figure}[H]
                        \centering
                        \begin{tikzpicture}
                            \begin{axis}[
        ...
        \subsubsection{Convoluzione}\label{Convoluzione}
            ...
            \end{itemize}
            Tutte le proprietà sono valutate nel dominio del $tempo$ ma valgono anche per il dominio della $frequenza$.
        ...
        \begin{itemize}
            ...
            \item {
                $t=\frac{T}{2}$: le due $rect$ sono disgiunte, nel raggiungere questa posizione il valore dell'integrale è diminuito fino a $0$.
            }
        \end{itemize}
        Tracciamo l'andamento dell'integrale:
        \begin{figure}[H]
            \centering
            \begin{tikzpicture}
                \begin{axis}[
        ...
        \begin{itemize}
            ...
            \item {In $t=0$ è l'area il prodotto dei segnali}
        \end{itemize}
        ...
        \begin{figure}[H]
            ...
            \caption{Integrale di convoluzione}
        \end{figure}
        Dal Th. di Convoluzione \ref{Convoluzione} sappiamo che è il prodotto di convoluzione di 2 rect di durata uguale a $T$:
        \begin{gather}
    ...
    \subsection{Modulazione di Ampiezza}\label{Modulazione di Ampiezza}
        ...
        Se il segnale non fosse modulato si dice in {\color{blue} \textbf{banda base} (BB)} se il segnale è modulato si dice in {\color{red}\textbf{banda passante}(BP)}.
        \begin{figure}[H]
            \centering
            \begin{tikzpicture}
                \begin{axis}[
        ...
        \subsubsection{Th. Modulazione con $\cos(2\pi f_0t + \phi)$}\label{Modulazione con coseno generico}
            ...
            \end{figure}
        % È legale cosa ho scritto sopra della modulazione generica?
        \subsubsection{Th. Modulazione con Esponenziale Complesso}\label{Modulazione con Esponenziale Complesso}
            ...
            \end{align}
            Dall'ultima uguaglianza posso quindi usare un filtro in ({\color{blue}BB}) per rimuovere i segnali alle frequenze $\pm 2f_0$ e ricavare il mio segnale $G_{(f)}$.\\
            Riporto un po di esempi di demodulazione di una o più $rect$:
            ...
            \end{figure}
            Nel caso di più segnali durante la demodulazione il segnale che voglio recuperare viene spostato in {\color{blue}BB}
            mentre gli altri segnali presenti sullo spettro vengono a loro volta modulati però con un $f_0$ diverso rispetto al loro $f_0^\prime$:
            \begin{figure}[H]
                \centering
            ...
            \end{figure}
            Potrei però trovarmi in situazioni delle quali non posso recuperare il segnale
            \begin{figure}[H]
                \centering
                \subfloat[Due $sinc$ modulate]{
                    \begin{tikzpicture}
            ...
            \end{figure}
            Come possiamo vedere nella regione della {\color{blue}BB}, il segnale è molto sporco, magari può essere confuso con un 
            $\cos$ e non una sinc. inoltre ora qui ho usato numeri interi per fare il $plot$ quindi non si accavallano così male 
            ma si accavallano solo in $\frac{1}{T}$ se usassi altri valori sarebbe ancora più sporco il segnale.\\
            ...
            \subsubsection{Radar}
                ...
                Il calcolo della distanza si basa sulla capacità dei materiali di riflettere le onde, più la frequenza dell'onda è alta 
                più i materiali riescono a riflettere. L'emettitore, in presenza di ostacolo, riceve il segnale riflesso ({\em eco})
                ...
                \end{figure}    
                Indichiamo con $\tau$ il ritardo di ricezione dell'{\em eco} e la distanza che separa il radar dall'oggetto $d$:
                \[
                    d=\frac{c\tau}{2}
                \]
                Compare un fattore $\frac{1}{2}$ dovuto al segnale che percorre due volte la distanza tra l'emettitore e l'oggetto.
                Possiamo notare che l'{\em eco} ha ampiezza minore poiché dell'energia è stata assorbita dal materiale o una porzione del segnale
                originale ha oltrepassato il materiale stesso.
                
                Analizziamo in frequenza cosa succede. 
                
                Volgiamo realizzare un radar a onda rettangolare che rileva a un massimo di $15$ metri di distanza e a un minimo di $1,5$ metri.
                ...
                frequenziali significative nell'ordine dei GHz, $\frac{1}{T}= \frac{1}{10^{-9}} = 1GHz$. I segnali nell'ordine dei GHz oltrepassano con facilità
                gli ostacoli, basti vedere le bande di funzionamento dei cellulari e il Wi-Fi. Si utilizzano segnali con l'ordine di centinaia se non migliaia di Hz, 
    ...
    \subsection{Delta di Dirac}\label{Delta di Dirac}
        ...
        Più correttamente si definisce Delta di Dirac la funzione che se integrata restituisce il gradino unitario:
        \[
        ...
        \end{figure}    
        L'ampiezza della funzione è dovuta alla costante moltiplicativa, ma alla fin fine rimane sempre una funzione impulso.\\

        % \textbf{boh amplia magari}
        
        \subsubsection{Proprietà del Delta di Dirac}\label{Proprietà del Delta di Dirac}
                \begin{itemize}
                    \item {
                        $\int_{-\infty}^{\infty} \delta_{(t)} dt = 1$
                    }\label{PD1}
                    \item {
                        Proprietà Campionatrice:\label{PDD Campionatrice}\index{Proprietà Campionatrice del Delta di Dirac}\\
                        ...
                        \begin{figure}[H]
                            ...
                            \begin{tikzpicture}
                                ...
                                \end{axis}
                            \end{tikzpicture}
                            \caption{Proprietà Campionatrice}
        ...
        \subsubsection{$TCF$ di segnali periodici}\label{TCF di segnali periodici}\index{TCF di segnali periodici}
                Nel caso di segnali periodici, come $x_{(t)} = \cos(2\pi f_0t)$, abbiamo definito la $TSF$, proviamo a calcolarne la $TCF$:
                \[
            ...
            \subsection{Relazione tra $TSF$ e $TCF$}
                \subsubsection{$TCF$ di un segnale periodico generico}\label{TCF di un segnale periodico generico}\index{TCF di un segnale periodico generico}
                    ...
                    \[
                        ...
                        \end{cases}
                    \]
                    La TCF diventa un campionamento con la Delta di Dirac \ref{Proprietà del Delta di Dirac}
                ...
                \subsubsection{Relazione tra i coefficienti $X_k$ e $X_{(f)}$}
                    ...
                    \begin{align}
                        Y_k &= \frac{1}{T_0} \int_{-\frac{T_0}{2}}^{\frac{T_0}{2}} y_{(t)} e^{j2\pi kf_0t} dt =\frac{1}{T_0} \int_{-\frac{T_0}{2}}^{\frac{T_0}{2}} \overunderset{+\infty}{n = -\infty}{\sum}x_{(t-nT_0)} e^{j2\pi kf_0t} dt \nonumber \\
                                ...
                                &= \frac{1}{T_0} \overunderset{+\infty}{n = -\infty}{\sum} \int_{-\frac{T_0}{2}-nT_0}^{\frac{T_0}{2}-nT_0} x_{(t^\prime)} e^{j2\pi kf_0t^\prime} \underset{\substack{\downarrow \forall nk=1 \\\text{è la fase di un numero sempre intero nk}}\in\mathbb{N}}{e^{j2\pi nkf_0T_0}} dt^\prime\nonumber \\
                ...
                \subsubsection{$I^a$ formula di Poisson}\label{prima formula di poisson}
                    ...
                    \end{figure}
                    ${\color{red}y_{(t)}} = |\cos(2\pi f_0t)|$ non è altro che il segnale aperiodico ${\color{blue}x_{(t)}} = \cos(2\pi f_0t) \dotproduct rect\left(\frac{t}{\frac{T_0}{2}}\right)$ con periodo $\frac{T_0}{2}$:
                    \[
                        \begin{cases}
                        ...
                        \end{cases}
                    \]
                    Per il calcolo di $X_{(f)}$ si applica il Th della Modulazione con $cos$ \ref{Modulazione con coseno} e la proprietà campionatrice della Delta di Dirac rispetto alla convoluzione \ref{PD3}.
                    \[
                    ...
                    \begin{itemize}
                        ...
                        \item{
                            ...
                            \begin{cases}
                                ...
                                X_{(f)} = 1 \nonumber    
                            \end{cases}$
                            la terza ipotesi non è mai verificata. Scriviamo la funzione gradino in modo diverso:
                            \[
