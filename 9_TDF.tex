\section{Trasformata Discreta di Fourier}
    ...
    \begin{gather}
        ...
        \end{cases} \text{è una funzione periodica di periodo }\frac{1}{T}\nonumber  
    \end{gather}
    ...
    \subsubsection{Relazione tra $TCF$ e $TDF$}\label{Relazione tra $TCF$ e $TDF$}
        \begin{align}
            \overline{X}_{(f)} &= \sum_{n=-\infty}^{\infty} x_{[nT]}e^{-j2\pi fnT} = \sum_{n=-\infty}^{\infty} \underset{(\nu)}{\int_{-\infty}^{\infty}} X_{(\nu)} e^{-j2\pi \nu nT} d\nu e^{-j2\pi fnT} \nonumber \\
                               ...
                               &\overset{\ref{Proprietà del Delta di Dirac}:\text{parità}}{=} \underset{(\nu)}{\int_{-\infty}^{\infty}} X_{(\nu)} \frac{1}{T} \sum_{n=-\infty}^{\infty}  \delta_{(\nu-(f-\frac{k}{T}))} d\nu = \frac{1}{T} \sum_{n=-\infty}^{\infty} \underset{(\nu)}{\int_{-\infty}^{\infty}} X_{(\nu)} \delta_{(\nu-(f-\frac{k}{T}))} d\nu  \nonumber \\
                               &\overset{\ref{Proprietà del Delta di Dirac}:\text{campionatrice}}{=}  \frac{1}{T} \sum_{n=-\infty}^{\infty} X_{(f-\frac{k}{T})} \nonumber
        \end{align}
    ...
    \subsection{Teorema del Campionamento - Nyquist Shannon}\label{Teorema del Campionamento - Nyquist Shannon}
        ...
        \end{itemize}
        Il nostro obbiettivo è dimensionare T e l'interpolatore in modo tale da poter ricostruire il segnale $\hat{x}:\ \hat{x} =x$.
        ...
        abbiamo trovato che la $TDF$  non è altro che la periodicizzazione della $TCF$ con periodo $\frac{1}{T}$: se $\frac{1}{T}\geq 2B$
        ...
        \end{figure}
        ho sovrapposizione delle copie di $X_{(f)}$ è il fenomeno di Aliasing: ho sovrapposizione di segnali che alterano il segnale originale. 
        ...
        \paragraph{Interpolatore a seno cardinale:} Si utilizza un interpolatore a seno cardinale
            ...
            \end{figure}
            l'interpolatore a seno cardinale non è causale: essendo delle $sinc$ a ogni campionamento, non posso conoscere il peso delle $sinc$ 
        ...
        il segnale $x_{(t)}$ può essere univocamente ricostruito a partire dai suoi campioni $x_{[nT]}$ presi a frequenza (o periodo) $f_s = \frac{1}{T}$ se 
