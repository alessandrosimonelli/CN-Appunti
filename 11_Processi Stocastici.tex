\section{Processi Stocastici}
    ...
    \subsection{Autocorrelazione}
        ...
        \]
        \subsubsection{Proprietà della autocorrelazione $R_x$}
            \begin{itemize}
        ...
        \subsubsection{Significato fisico dell'autocorrelazione}
            ...
            \end{figure}
    \subsection{Sistemi Stazionari in Senso Lato (SSL)}\label{Sitemi Stazionari in Senso Lato (SSL)}
    ...
    \subsection{Processo Gaussiano}
        ...
        \subsubsection{Rumore Bianco}
            È una sorgente di rumore idealizzata in modo tale da rendere lo spettro di densità di potenza costante ($\frac{N_0}{2}$), cioè indipendente
            dalla frequenza a cui operiamo. L'aggettivo "Bianco" deriva dall'ottica: un segnale di luce bianca contiene in ugual maniera tutte le frequenze
             ...
             \begin{figure}[H]
                \centering
                \subfloat[Densità di potenza spettrale]{
                    \begin{tikzpicture}
                        \begin{axis}[
                            xlabel=$f$,
                            ylabel=$S_{W(f)}$,
        ...
        \subsubsection{Filtro passa basso ideale per rumore bianco}
            ...
            \begin{figure}[H]
                ...
                \end{tikzpicture}
            \end{figure}
            per questo il modello $Y = S + n \sim \mathcal{N} (0,\sigma^2)$ è giusto per approssimare il rumore. Calcoliamone il valor medio e varianza:
            \begin{gather}
                w_{(t)} \sim \mathcal{N}(0,\sigma^2_w)  \nonumber \\
                \sigma^2_w = \mathbb{E}[X^2]-\mu_w = \mathbb{E}[X^2] = R_{w(0)} = N_0B\nonumber
            \end{gather}
            Ne segue che più la banda è elevata più rumore entra nel sistema.            
