\section{Sistemi Monodimensionali}
    ...
    \subsection{Proprietà dei Sistemi Lineari Tempo Invarianti (LTI)}
        \begin{itemize}
            \item {Linearità:
                \[
            ...
            \item {Causalità:
                ...
                \begin{itemize}
                    \item Real Time: necessariamente causale (è nel presente)
    ...
    \subsection{Proprietà dei Sistemi Lineari Stazionari (SLS)}
        ...
        \]
        La trasformata del sistema dipende solo dalla $h_{(t)}$ (risposta impulsiva del sistema)\index{Risposta Impulsiva}. Passando al dominio della frequenza per la proprietà della convoluzione \ref{Convoluzione}:
        \[
    ...
    \subsection{Risposta di un sistema causale e risposta impulsiva}
        ...
        \end{align}
        giungiamo all'integrale che definisce la Causalità \ref{SM Causalita}: un sistema è causale se la sua risposta impulsiva 
        è solo definita per $t>0$.
        \begin{figure}[H]
            \centering
            \begin{tikzpicture}
                \begin{axis}[
        ...
        \subsubsection{Stabilità BIBO su $h_{(t)}$}
            ...
            \end{align}
            Se l'ingresso $y<M$ allora lo è anche l'uscita. Ne deriva la C.S(Condizione Sufficiente): Assoluta integrabilità della $h_{(t)}$ \index{Assoluta integrabilità $h_{(t)}$},
    ...
    \subsection{Filtri Ideali}
        \paragraph{Banda di un segnale} È la porzione di spettro calcolata sul semiasse positivo delle
        ...
        \paragraph{Circuito Filtro Passa Alto:}
            \begin{figure}[H]
                \centering
        ...
        \subsubsection{Filtro Passa Banda di banda B - Band Pass Filter (BP)}\label{Filtro Passa Banda di banda B - Band Pass Filter (BP)}
            ...
            \paragraph{Circuito Filtro Passa Banda:}
                \begin{figure}[H]
                    \centering
            ...
            \end{figure}
            Non riesco a fare un grafico con pendenza maggiore ma è per capire che le frequenze sotto il valore di $-3db$
        ...
        \subsubsection{Filtri non distorcenti}
            Definiamo prima cos'è  un segnale non distorto:
            \begin{figure}[H]
                \centering
                \begin{tikzpicture}[
                    node distance=2cm,
            ...
            \end{figure}
            {\color{blue}$y_{(t)}$} è un segnale non distorto di $x$ se: $y_{(t)}\triangleq kx_{(t-t_0)}$ $k\in\mathbb{R}^+$, cioè finché esso rimane una versione
    ...
    \subsection{Th. di Parseval}\label{Th. di Parseval}
        ...
        \end{figure}            
        \noindent La parte {\color{green}verde} è l'energia del segnale che perdiamo troncandolo con banda $B$
    ...
    \subsection{Funzione di Autocorrelazione}\label{Funzione di Autocorrelazione}
        ...
        \subsubsection{Proprietà Autocorrelazione}
            \begin{itemize}
                \item {$R_{x(0)} = E_x$
                    \[
